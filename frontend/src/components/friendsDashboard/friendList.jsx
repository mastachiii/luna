    ...
    return (
        <dialog ref={ref} className={`m-auto  rounded-md`}>
            ...
            <div className="h-15 flex justify-end items-center gap-5 mt-5 pr-4 bg-zinc-100">
                <button onClick={() => ref.current.close()} className="text-sm cursor-pointer hover:underline">Cancel</button>
                <button onClick={handleRemoveFriend} className="p-2 pl-3 pr-3 text-sm text-white rounded-sm  bg-red-500 cursor-pointer hover:bg-red-700">Remove Friend</button>
            </div>
        </dialog>
    );
}
